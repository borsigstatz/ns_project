...
#Data
#-------------------------------------
DF3=pd.read_json(DATA_PATH.joinpath('MOTIV_Data.json.gz'))
df_table3=pd.read_json(DATA_PATH.joinpath('MOTIV_ShortDesc.json.gz'))
...
data_table= dash_table.DataTable(
                            ...
                            columns=[
                                {"name": i, 
                                 "id": i, 
                                 "deletable": False,
                                 "selectable": True} for i in df_table3.columns],
                            style_table={
                                'maxHeight': '300ex',
                                'overflowY': 'scroll',
                                'overflowX':'auto',
    ...
    [
     ...
     dbc.Row( #Row 2 -> 4 + 5 +1
         [
             dbc.Col(
                [
                html.H1('MOTIV-Dataset'),
                html.H2('Codebook'),
                html.P('This part contains general Information about the Motiv-Dataset'),
                ], 
             ...
             dbc.Col(
                dbc.Spinner(html.Div(id='miss-info3')),
     ...
     dbc.Row(
         [
             ...
             dbc.Col(dbc.Spinner(html.Div(id='plot-area3')),
...
@app.callback(
    ...
    [Input('datatable-interactivity3', 'selected_rows')]
)
...
def update_miss_case(rows,derived_virtual_selected_rows, selected_rows):
    ...
    if derived_virtual_selected_rows:
        ...
        VAR_lab=meta_list['MOTIV_VarLab']
        VAL_lab=meta_list['MOTIV_ValueLab']
...
#--------------------------------------
@app.callback(
    Output('var-description3', "children"),
    [Input('datatable-interactivity3', "derived_virtual_data"),
     Input('datatable-interactivity3', "derived_virtual_selected_rows"),
     Input('tabs-for-desc3', 'active_tab'),
     ], #Input('memory-output', "data")
     [State('datatable-interactivity3', 'selected_rows')]
    )
...
def update_description(rows,derived_virtual_selected_rows, at, selected_rows):
    ...
    if derived_virtual_selected_rows:
        ...
        VAR_lab=meta_list['MOTIV_VarLab']
        VAL_lab=meta_list['MOTIV_ValueLab']
        description=meta_list['MOTIV_LongDesc']
        stats=help_functions.save_stats(DF,var, VAR_lab, VAL_lab,
                         missings=[x for x in range(-9,1)])
...
#----------------------
@app.callback(
    Output('int_plot3', "figure"),
    [Input('int_radio3', 'value')], #Input('memory-output', "data")
    [State('datatable-interactivity3', 'selected_rows')]
    )
def update_int_graph(value, selected_rows):
    ...
    df_table=df_table3
    VAR_lab=meta_list['MOTIV_VarLab']
    VAL_lab=meta_list['MOTIV_ValueLab'] #VAL LAB is none
    var=df_table.iloc[selected_rows[0],0]
    print(VAL_lab[var])
    stats=help_functions.save_stats(DF,var, VAR_lab, VAL_lab,
                     missings=[x for x in range(-9,1)])
...
#-----------------------------
@app.callback(
    Output('str_plot3', "figure"),
    [Input('str_plot_slider3', 'value'),
      ], #Input('memory-output', "data")
